...
The Anthropic API key is still useful if you ever want to re-enable the `citation-probe.yml` workflow (which talks to non-Anthropic LLMs and needs separate API keys per provider).
